...
### 1.1 抓取 + 打分（fetch\_and\_score.py）

数据源：

- DBLP proceedings / journal pages：顶会与期刊目录页
- conference program pages：最近会议的 technical sessions / program 页面
- arXiv API：`https://export.arxiv.org/api/query`，搜索 `cs.AR, cs.DC, cs.NI, cs.OS, cs.PL, cs.AI, cs.LG`
- Semantic Scholar API：候选不足时补充

打分规则：

- 命中 `negative_keywords` → 直接 -999 排除
- 命中 `keywords`：标题 +3，摘要 +1
- 命中 `domain_boost_keywords`：按 systems 相关性额外加分
- 命中顶会/期刊来源：可信度加分，但不能覆盖主题无关性

去重：

- 按 arXiv ID 或标题合并，保留高分
...
3. 自动分类到 systems 概念子目录（架构 / 内存存储 / 网络互连 / 分布式 / 编译运行时 / 性能评测）
4. 创建概念笔记：定义 + 数学形式 + 核心要点 + 代表工作 + 相关概念

### 3.2 论文笔记生成

- 只为"🔥 必读"论文生成完整笔记
- 已有笔记如果 < 100 行或缺少 `批判性思考 / 关联笔记 / 速查卡片` → 删除重新生成
...
- 包含 LaTeX 公式（≥ 2 处）
- 包含图片引用（≥ 1 处）
- 包含 `## 批判性思考`、`## 关联笔记`、`## 速查卡片` section，且不应再有独立 `## 关键公式` / `## 关键图表`
- 不达标 → 删了重来
...
- 不确定分类 → `_inbox/`
- YAML frontmatter：title / method\_name / authors / year / venue / tags / zotero\_collection / image\_source / created
...
### 批量处理（paper\_daemon.py）

```bash
python3 paper_daemon.py -c "Distributed Systems"  # 处理系统方向分类
...
```json
{
  ...
  "daily_papers": {
    "keywords": ["computer architecture", "rdma", "llm serving", ...],
    "negative_keywords": ["medical imaging", "weather forecast", ...],
    "domain_boost_keywords": ["gpu", "cluster", "interconnect", ...],
    "arxiv_categories": ["cs.AR", "cs.DC", "cs.NI", "cs.OS", "cs.PL", ...],
    "min_score": 2,
    "top_n": 30
  },
  "automation": {
...
│   │   ├── 1-Computer Architecture and Accelerators.md
│   │   └── FlashAttention.md
│   ├── 4-Distributed Systems/
│   │   ├── 4-Distributed Systems.md
│   │   └── vLLM.md
│   ├── _概念/
│   │   ├── 1-Computer Architecture and Accelerators/
│   │   │   └── Tensor Core.md
