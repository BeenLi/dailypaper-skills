...
8. **批判性思考**: 优点、局限与隐含假设、可能失效场景、潜在改进方向
9. **经验与复现**: 单独写 `经验与可迁移启示` 和 `复现`；复现包含环境依赖、关键配置 / workload / data、`复现核查表`、风险与缺口；核查表使用普通 Markdown table 或普通 bullet，禁止使用 `- [ ]` / `- [x]` task list
10. **关联笔记分类**: 分为 "方法相关"、"理论 / 指标相关"、"系统 / 硬件相关"
11. **速查卡片**: Obsidian `[!summary]` 格式的快速参考
12. **结构约束**: 不要单独写 `## 关键公式` 或 `## 关键图表`；公式和图片必须嵌入“问题定义与瓶颈 / 系统设计总览 / 关键机制拆解 / 实验设置 / 核心结果 / Overhead 与兼容性”这些章节里
13. **相关工作定位**: 表头使用“论文 / 外部链接 / 关系 / 差异”；`论文` 列优先写 `[[笔记名]]`，没有内部笔记时写纯标题；`外部链接` 列写 `[arXiv](...)` / `[DOI](...)` / `[Paper](...)`，没有就留空
