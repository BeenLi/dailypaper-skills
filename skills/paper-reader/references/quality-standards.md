...
- 图表应嵌入它支撑的语义小节下，标题用加粗图注行：`**Figure X: ...**` / `**Table X: ...**`。
- `###` 只给真实语义结构使用，例如 `### 动机实验`、`### Encoder 消融`、`### SLO 适配结果`、`### 附录补充图`。
- 多张低优先级附录图可以合并在一个语义小节下，逐张用加粗图注行列出。
- 如果某张图本身就是一个核心机制小节的唯一内容，先给机制小节起语义标题，再在小节内放 Figure 图注。
