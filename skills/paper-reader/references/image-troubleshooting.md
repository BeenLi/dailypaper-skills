...
mkdir -p {笔记所在目录}/00_assets/
pdfimages -png /tmp/paper.pdf /tmp/{note-name}_fig
```
从 PDF 裁切 / 提取后，将图片保存为 `{笔记所在目录}/00_assets/<note-name>_<原图片名>` 并验证：文件 >10KB、Read 确认内容正确。

本地图片命名规则：
- `note-name ≤ 48 字符`：直接用 `<note-name>_` 前缀。
...
- **可达** → 保持外链不动
- **不可达** → 下载到 `00_assets/<note-name>_<原图片名>`，替换为 `![[00_assets/<note-name>_<原图片名>]]` wikilink
...
**本地**（外链不可用时的备选）:
```markdown
![[00_assets/{note-name}_fig1_overview.png]]
```
...
```yaml
---
image_source: online  # 默认 online；部分图片用本地时填 mixed；全部来自本地 PDF 时填 local
arxiv_id: "2501.12345"
---
```
